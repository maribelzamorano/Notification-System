 ...
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (020) notifadm.pss_EVST_IUD.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('notifadm.pss_EVST_IUD') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure notifadm.pss_EVST_IUD
	as begin
...
alter procedure notifadm.pss_EVST_IUD
	@SESSION_ID	UDT_SESSION_ID,		
	@USER		UDT_USER,			

	@EVST_ID	UDT_ID output,		
	@EVST_USER	nvarchar(128),		
	@EVST_XPTS	nvarchar(23),		
...
--> 
	...
	begin
		...
		if ( @AAKT_CODE in ( 20, 30, 90 )) and 
		   ( not exists( select 1 from notifadm.IO_EVENT_SUBSCRIPTION_TYPE where EVST_ID = @EVST_ID and EVST_USER = @EVST_USER and EVST_#PTS = @EVST_XPTS ))
		begin
			set @rc = -1;
...
--				end
				...
				begin
					...
					insert into notifadm.IO_EVENT_SUBSCRIPTION_TYPE(
						--EVST_ID,	-- only of psp_getIdentity() is used
						SBTP_CODE,EVTP_CODE,
						EVST_USER, EVST_PTS, EVST_#PTS,
						--EVST_ID_INT,	-- only of psp_getIdentity() is used
						EVST_UQID )
					values(
						--@id,
						@SBTP_CODE,@EVTP_CODE,
					...
					select	@EVST_ID = EVST_ID
					from	notifadm.IO_EVENT_SUBSCRIPTION_TYPE
					where	EVST_UQID = @uqid;

					if ( @@rowcount <> 1 ) or ( isnull( @EVST_ID, 0 ) = 0 )
					begin
						set @rc = -1;
			...
			begin
				...
				update notifadm.IO_EVENT_SUBSCRIPTION_TYPE set
					...
					EVST_USER			= @USER,
					EVST_PTS			= @pts,
					EVST_#PTS			= services.pfn_getXPTS( @pts )

				where EVST_ID = @EVST_ID and EVST_USER = @EVST_USER and EVST_#PTS = @EVST_XPTS;
			...
			begin
				...
				delete from notifadm.IO_EVENT_SUBSCRIPTION_TYPE where EVST_ID = @EVST_ID and EVST_USER = @EVST_USER and EVST_#PTS = @EVST_XPTS;
